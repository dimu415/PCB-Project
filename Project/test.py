...
class MyWindow(QMainWindow):
    ...
    def Main_UI(self):
        ...
        buttons = [Upload_btn, label_btn, split_btn,agu_btn, train_btn, ana_btn]        
...
# mainwindow
        ...
        Upload_lb = QLabel("")
        Upload_lb.setStyleSheet("background-color: rgba(128, 128, 128, 150); ")
        font = QFont("Broadway", 50, QFont.StyleNormal)
        Upload_lb.setFont(font)
        Upload_lb.setFixedSize(600, 400)
        ...
        self.file_name=""
        upload_vlayout.addWidget(load_file)
        upload_vlayout.addWidget(Upload_lb)
        upload_vlayout.addWidget(self.file_name_line)
        upload_vlayout.setSpacing(20)
        upload_vlayout.setAlignment(Qt.AlignTop)
        
        upload_vlayout2 = QVBoxLayout()
        load_ano=QPushButton("불러오기")
        load_ano.clicked.connect(self.Load_ano)
        load_ano.setStyleSheet("background-color:#673AB7;"
                            "color:white;"
                            "font-size: 18px;"
                            "font-family: Arial, sans-serif;"
                            "font-weight: bold;"
                            "border-radius: 20px;")
        load_ano.setFixedSize(200,50) 
        
        em=QLabel()
        em.setFixedSize(100,60)
        self.load_df_widget = QTableWidget()
        self.load_df_widget.setFixedSize(600, 400)
        check_btn=QHBoxLayout()
        empty=QLabel("")
        empty.setFixedWidth(380)
        ok_btn=QPushButton("Select")
        ok_btn.setStyleSheet('background-color:#7E57C2;'
                             'border-radius: 10px;'
                             )
        ok_btn.clicked.connect(self.xml_to_txt)
        ok_btn.setFixedSize(100, 30)
        #ok_btn.setFixedWidth(100)
        del_btn=QPushButton("Delete")
        del_btn.setStyleSheet('background-color:#7E57C2;'
                              'border-radius: 10px;')
        del_btn.clicked.connect(self.Create_yaml)
        del_btn.setFixedSize(100, 30)
        del_btn.setFixedWidth(100)
        check_btn.addWidget(empty)
        check_btn.addWidget(del_btn)
        check_btn.addWidget(ok_btn)
        
        upload_vlayout2.addWidget(load_ano)
        upload_vlayout2.addWidget(self.load_df_widget)
        upload_vlayout2.addLayout(check_btn)
        upload_vlayout2.setAlignment(Qt.AlignTop)
        upload_vlayout2.setSpacing(10)
        self.Upload_layout.addLayout(upload_vlayout)
        self.Upload_layout.addLayout(upload_vlayout2)
        self.Upload_layout.setSpacing(50)
        #%% label
        ...
        self.label_ls.setFixedSize(200, 700)
    ...
    #%% split
        ...
        #file_gb
        file_gb=QGroupBox()
        file_gb.setStyleSheet("QGroupBox { border: 1px solid gray; border-radius: 10px; }")
        file_header_label = QLabel("파일명", file_gb)
        file_header_label.setFixedSize(800,30)
        file_header_label.setAlignment(Qt.AlignCenter)
        file_header_label.setStyleSheet("background-color: #673AB7; color: white; padding: 5px;"
                                        "border-top-left-radius: 10px; border-top-right-radius: 10px;")
        file_gb_layout = QHBoxLayout()
        ch = QCheckBox("이름")
        file_gb_layout.addWidget(ch)
        file_gb.setLayout(file_gb_layout)  
        file_gb.setFixedSize(800, 110)
        ...
        self.split_result_label = QLabel('',self)
        ...
        split_vlayout.addWidget(file_gb)
    ...
    #%% train
        ...
        train_vlayout2 = QHBoxLayout()
        train_result_Box = QGroupBox()
        #train_result_Box.setFixedSize(200,130)
        ...
        #train_result_label.setFixedSize(200,30)
    ...
    #%% Analyze
        ...
        self.sidebar_widget = QWidget()
        self.setCentralWidget(self.sidebar_widget)
        self.sidebar_widget.setStyleSheet("border-left: 10px solid #1A2D3A; border-right: 10px solid #1A2D3A; background-color: #121D27;")
...
#%% part -Upload
    ...
    def xml_to_txt(self):
        ...
        xml_to_txt_fun(self.ano_paths, output1)
    ...
    def Load_df(self, df):
        self.load_df_widget.setRowCount(df.shape[0])
        self.load_df_widget.setColumnCount(df.shape[1])
        self.load_df_widget.setHorizontalHeaderLabels(df.columns)

        for row in range(df.shape[0]):
            for col in range(df.shape[1]):
                item = QTableWidgetItem(str(df.iat[row, col]))
                self.load_df_widget.setItem(row, col, item)
#%% part-label
    def List_view(self):
        if len(self.df) !=0:
                        ...
                        with open(file_path, "r") as file:
                            for line in file:
                                try:
                                    ...
                                    self.class_name[str(line.split(" ")[0])]=str(line.split(" ")[0])
            ...
            for n,f in enumerate(self.class_lb):
                item=f+f":{f}:"+str(self.class_lb[f])
    ...
    def image_label(self, image_path, xml_path):
        ...
        with open(xml_path, "r") as file:
            for line in file:
                ...
                name_text = img_draw.textbbox((xmin - 175, ymin - 20),self.class_name[a[0]], anchor='lt', font=font)
                img_draw.rectangle([(name_text[0], name_text[1]), (name_text[2], name_text[3])], fill='red')
